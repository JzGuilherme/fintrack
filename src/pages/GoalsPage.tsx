  ...
  return (
    <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
        <h1 style={{ fontSize: '28px', fontWeight: 600, color: colors.text }}>Metas</h1>
        <Button label="+ Nova Meta" onClick={() => setIsModalOpen(true)} variant="primary" />
      </div>

      {goals.length === 0 ? (
        <p style={{ color: colors.textSecondary, textAlign: 'center', padding: '2rem' }}>Nenhuma meta registrada</p>
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '24px' }}>
          {goals.map((goal) => {
            ...
            return (
              <div key={goal.id} style={{ backgroundColor: colors.surface, borderRadius: '8px', padding: '24px', boxShadow: '0 2px 8px rgba(0,0,0,0.08)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '16px' }}>
                  <h3 style={{ fontSize: '18px', fontWeight: 600, color: colors.text }}>{goal.title}</h3>
                  <Button label="Deletar" onClick={() => handleDelete(goal.id)} variant="danger" />
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                  <span style={{ fontSize: '14px', color: colors.textSecondary }}>{formatCurrency(goal.current)} de {formatCurrency(goal.target)}</span>
                  <span style={{ fontSize: '14px', fontWeight: 600, color: isCompleted ? '#22c55e' : colors.primary }}>{Math.min(Math.round(percentage), 100)}%</span>
                </div>
                <div style={{ width: '100%', height: '8px', backgroundColor: colors.border, borderRadius: '4px', overflow: 'hidden' }}>
                  <div style={{ height: '100%', width: `${Math.min(percentage, 100)}%`, backgroundColor: isCompleted ? '#22c55e' : colors.primary, transition: 'width 0.3s ease' }} />
                </div>
